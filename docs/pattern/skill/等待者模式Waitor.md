...
对于等待者对象你可以看出,其内部定义了三个数组,分别是监控对象容器,以及成功与失败回调函数容器:一个类-监控对象,该对象有两个属性,即监控解决成功状态与监控解决失败状态,两个方法,解决成功方法与解决失败方法:一个私有方法_exec来处理成功失败回调函数的方法:3个共有方法接口:when方法监控异步逻辑,done方法添加成功回调函数, fail方法添加失败回调函数。”小铭喘口气,歇了一下继续说,“等待者对象内部的属性我们己经定义好了,下面我们一一实现这些方法吧,首先我们还是完成监控对象类的原型方法resolve和reject。
